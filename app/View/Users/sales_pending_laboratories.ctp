...
<div class="content">
	<div id="main-tab" class="widget-result">
        <ul class="tabnav">
             <li><a style="background: none;"  href="#laboratories">Pending Laboratories</a></li>
        </ul>
				...
				<tbody>
					<?php foreach($users as $user):?>
						...
						<tr>
							...
							<td><?php echo $user['User']['Laboratory']['class'];?></td>
							<td>
								<?php echo $this->Html->link('View',array('controller' => 'users','action' => 'lab_profile','sales' => true,$company['CompanyBranch']['id']));?>
								<?php echo $this->Form->postLink(__('Activate'), array('controller' => 'users','action' => 'lab_activate', 'sales' => true, $user['User']['id']), null, __('Are you sure you want to activate user %s?', $user['User']['username'])); ?>
							</td>
						</tr>
					<?php endforeach;?>
				</tbody>
