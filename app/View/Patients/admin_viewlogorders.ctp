		...
		<table id="common-tbl" class="visits_tbl" >
			<thead>
				...
				<th style="width:20%;"><?php echo $this->Paginator->sort('url', 'Action Url'); ?></th>
				 --><th style="width:55%;"><?php echo $this->Paginator->sort('remarks', 'Episode Numbers'); ?></th>
				<th style="width:15%;"><?php echo $this->Paginator->sort('device', 'Device'); ?></th>
				<th style="width:15%;"><?php echo $this->Paginator->sort('browser', 'Browser'); ?></th>
				
			</thead>
			<tbody>
			<?php foreach ($auditlogsloadorders as $auditlog):?>
				<tr>
					...
					foreach (json_decode($auditlog['AuditLog']['remarks']) as $key=>$remark){
						...
						if(is_array($remark)){
							foreach ($remark as $data){
								if($count < 4){
									$specimenids.=trim(preg_replace('/\s+/', '', $data)).", ";
									$count++;
								}else{
									$specimenids.=trim(preg_replace('/\s+/', '', $data)).",<br>";
									$count=0;
								}
							}
							echo substr_replace($specimenids, "", -2);//substr($specimenids, 0, -1);
							
							
						}else{
							echo trim(preg_replace('/\s+/', '', $remark))."";
						}						
					}?>
