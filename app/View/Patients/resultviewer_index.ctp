						...
						<!-- <div id="main-tab" class="widget-result"> -->
							<div id="patientsearch-ouput" class="tabdiv" style="border:5px solid #EC7D7D; padding: 0px; margin-top: 0px !important;">
				            	<div id="search-output">
				                	...
				                	<table id="common-tbl" class="patient_search visits_tbl" style="/*width:220px;*/ border: 0px solid white;" >
					                		<tbody>
					                		</tbody>
					                	</table>
				                	</div>
...
<script>
	...
	var prevhandler = false;
	var prevhandler2 = false;
	jQuery(document).ready(function(){
    	...
    	jQuery('#patientlist').css({'height':windowheight-(windowheight*0.63)});
...
// 				jQuery('#pdfFrame iframe#iframeId').attr('src','/physicians/viewPdfMro/'+labid+''+specimenid);
				...
				if(localserver==0){
					jQuery.ajax({
		            //    url: '/patients/viewPdf/'+specimenid,
						url: '<?php echo $this->Html->url(array('controller'=>'patients','action'=>'viewPdf','resultviewer'=>true));?>',
						data:{'Patient':{'specimenid':specimenid}},
						type: 'POST',
						dataType : 'json',
						asynch:false,
						success:function(data){
		                    if(data){
							//alert(url+data);
								url = url.replace("/Patients","");
		                    	remsid = specimenid;
		                    	jQuery('iframe#iframeId').height(windowheight-125);
		                    	jQuery('#pdfFrame iframe#iframeId').attr('src','http://docs.google.com/viewer?url='+url.replace("resultviewer","")+data+'&embedded=true');
		                    }else{
...
// 			alert(jQuery(this).find('td:nth-child(2)').text());
		...
		jQuery('.search-patient-btn').click(function(){
			...
			jQuery.ajax({
				...
				url: '<?php echo $this->Html->url(array('controller'=>'Patients','action'=>'searchPatient','resultviewer'=>false));?>',
				...
				success:function(data){
					...
					jQuery('.patient_search.visits_tbl tbody').empty();
					obj = data.Patient;
					if(obj){
...
//	 						tr += '<tr onclick="showPatientProfile('+y['Person']['internal_id']+')" style="cursor: pointer;"><td>'+y['Person']['internal_id']+'</td><td>'+ name +'</td><td>'+ y['Person']['birthdate'] +'</td><td>'+laboratories[y['Person']['laboratory_id']]+'</td></tr>';
						...
						if(data.Start > 1){
							paging += '<div style="text-align:center;"> <a id="'+ jQuery('input#PatientLastname').val() + ":" + data.Start + ":" + data.LastPaperId + ":prev" +'"> &lt;&lt;Previous </a>';
						}else{
							paging += '<div style="text-align:center;"> <a id="empty" class="disabled" style="color:gray;cursor:none;"> &lt;&lt;Previous </a>';
						}
						/* for (i = data.Start; i < data.Start + 10; i++) { 
							paging += '<a id="'+ jQuery('input#PatientLastname').val() + ":" + i + ":" + data.LastPaperId +'">'+i+'</a>&nbsp;';
						} */
						if((parseInt(data.Count) + 50) > data.Start){
							paging += '<a id="'+ jQuery('input#PatientLastname').val() + ":" + data.Start + ":" + data.LastPaperId + ":next" +'"> Next&gt;&gt; </a></div>';
						}else{
							paging += '<a id="empty" class="disabled" style="color:gray;cursor:none;"> Next&gt;&gt; </a></div>';
						}

						jQuery('div#paging').empty();
						jQuery('div#paging').append(paging);
						jQuery('.patient_search tbody').append(tr);
// 						jQuery('#patient-container div table ').trigger('live');
						//jQuery(document).live("click", "div#paging div span");
				...
				}, timeout: 300000 // sets timeout to 30 seconds
			});
			return false;
		});

		jQuery(document).on("click", "div#paging div a", function() {
			paramstr = jQuery(this).attr('id').split(":");
			
			if(paramstr[3]){
				name = paramstr[0];
				start = paramstr[1];
				lastpaperid = paramstr[2];
				action = paramstr[3];			
				getPaginatePatients(name, start, lastpaperid, action);
			}
		});
		jQuery('div#paging div a').unbind();
	...
	function getPaginatePatients(name, starttmp, lastpaperid, action){
	
		if(action == "prev"){
			if(!prevhandler){
				start = (parseInt(starttmp) - 50);
				prevhandler = true;
			}else{
				if(!prevhandler2){
					start = (parseInt(starttmp) - 50);
		...
		showLoadingMask(" Retrieving Data, please wait.. ");
		jQuery.ajax({
		url: '<?php echo $this->Html->url(array('controller'=>'Patients','action'=>'searchPatientByName','resultviewer'=>false));?>',
			data:{'Patient':{'lastname':name, 'start':start, 'action':action, 'lastpaperid':lastpaperid}},
			type: 'POST',
			dataType : 'json',
			error: function(){
		        alert('Connection timeout. Please try again later.');
...
//						jQuery('#patient-container div table ').trigger('live');
					//jQuery(document).live("click", "div#paging div span");
					handler = false;
					/* jQuery(document).on("click", "div#paging div a", function(event) {
							getPaginatePatients(jQuery(this).attr('id'), jQuery(this).attr('class'));
