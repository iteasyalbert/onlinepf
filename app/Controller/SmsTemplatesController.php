...
class SmsTemplatesController extends AppController {
	...
	function getSmsTemplates($page=null){
		$this->layout = false;
		...
		try {
            ...
            $response = $HttpSocket->get(Configure::read('api.domain_name').'/api/sms_template/index?page='.$page, $data,$request);
            $this->log(json_decode($response), 'apirespo_sms_templates');
            ...
            if($decoded_respo->success){
            	...
            	$send_audit = $this->addAuditLog('sms_templates',array(
					'success'=>true,
			...
			$myrequest['error']['status'] = true;
			$this->log($e->getMessage(), 'apirespo_sms_templates');
		}
	...
	public function admin_edit($id = null) {
    	...
    	if($this->request->is('get')){
	        try {
	            ...
	            // $data = json_encode($data);
	            $response = $HttpSocket->get(Configure::read('api.domain_name').'/api/sms_template/edit?id='.$id, $data,$request);
	            $this->log(json_decode($response), 'apirespo_sms_template_get');
	            $decoded_respo = json_decode($response);
	            $myrequest['data'] = $decoded_respo->data;
	            
			} catch (Exception $e) {
				$this->log($e->getMessage(), 'apirespo_sms_template_get');
    		...
    		try {
	            ...
	            $data = json_encode($data);
	            $response = $HttpSocket->post(Configure::read('api.domain_name').'/api/sms_template/edit', $data,$request);
	            $this->log(json_decode($response), 'apirespo_sms_template_edit');
	            $decoded_respo = json_decode($response);

	            $send_audit = $this->addAuditLog('sms_templates',array(
            		'url'=>'sms_templates/edit',
	            ...
	            if(!$decoded_respo->success){
                	...
                	$this->redirect('/admin/sms_templates');
	            }else{
	            	// todo : audit log here
				...
				$this->log($e->getMessage(), 'apirespo_sms_template_edit');
				$this->render('/Errors/error400', 404);
			}
    	}
        // if (!$this->User->exists()) {
