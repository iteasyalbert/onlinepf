 ...
 * @property User $User
 */
class ConsultantTypesController extends AppController {
	var $name = 'ConsultantTypes';
	...
	function getConsultantTypes($page=null){
		$this->layout = false;
		...
		try {
            ...
            $response = $HttpSocket->get(Configure::read('api.domain_name').'/api/consultant_type/index?page='.$page, $data,$request);
            $this->log(json_decode($response), 'apirespo_get_consultant_type');
            ...
            if($decoded_respo->success){
            	...
            	$send_audit = $this->addAuditLog('get_consultant_type',array(
					'success'=>true,
			...
			$myrequest['error']['status'] = true;
			$this->log($e->getMessage(), 'apirespo_get_consultant_type');
		}
	...
	function admin_add(){
		...
		try {
            ...
            $data = json_encode($data);
            $response = $HttpSocket->post(Configure::read('api.domain_name').'/api/consultant_type/add', $data,$request);
            $this->log(json_decode($response), 'apirespo_add_consultant_type');
            $decoded_respo = json_decode($response);
            if($decoded_respo->success){
            	$filter = "";
				foreach ($this->data as $key => $value) {
            	...
            	$send_audit = $this->addAuditLog('add_consultant_type',array(
					'success'=>true
				));
		    	try {
					ini_set('default_socket_timeout', 10);
	...
	public function admin_edit($id = null) {
    	...
    	if($this->request->is('get')){
	        try {
	            ...
	            // $data = json_encode($data);
	            $response = $HttpSocket->get(Configure::read('api.domain_name').'/api/consultant_type/edit?id='.$id, $data,$request);
	            $this->log(json_decode($response), 'apirespo_consultant_type_get');
	            $decoded_respo = json_decode($response);
	            $myrequest['data'] = $decoded_respo->data;
			} catch (Exception $e) {
				$this->log($e->getMessage(), 'apirespo_consultant_type_get');
    		...
    		try {
	            ...
	            $data = json_encode($data);
	            $response = $HttpSocket->post(Configure::read('api.domain_name').'/api/consultant_type/edit', $data,$request);
	            $this->log(json_decode($response), 'apirespo_consultant_types_edit');
	            $decoded_respo = json_decode($response);

	            $send_audit = $this->addAuditLog('consultant_types',array(
            		'url'=>'consultant_types/edit',
	            ...
	            if(!$decoded_respo->success){
                	...
                	$this->redirect('/admin/consultant_types');
	            }else{
	            	// todo : audit log here
				...
				$this->log($e->getMessage(), 'apirespo_consultant_types_edit');
				$this->render('/Errors/error400', 404);
			}
    	}
        // if (!$this->User->exists()) {
